...
<Note>
  ...
  we wrapped `getPosts` in `cache`. `getPost` just calls `getPosts`, so we're
  not making any unnecessary requests to the filesystem (or wherever you're
  getting your posts from).
</Note>
